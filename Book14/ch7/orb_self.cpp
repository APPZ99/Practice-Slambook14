...
// 描述子
typedef vector<uint32_t> DescType;  // 8*32=256
...
int main(int argc, char** argv)
{
    ...
    vector<cv::KeyPoint> keypoints_1, keypoints_2;  // 关键点
    vector<DescType> descriptor_1, descriptor_2;    // 描述子
    cv::FAST(first_image, keypoints_1, 40);
    cv::FAST(second_image, keypoints_2, 40);
    ...
    // 画出匹配图
    cv::Mat image_show;
    cv::drawMatches(first_image, keypoints_1, second_image, keypoints_2, matches, image_show);
...
void ComputeORB(const cv::Mat &img, vector<cv::KeyPoint> &keypoints, vector<DescType> &descriptors)
{
    ...
    for(auto &kp : keypoints)
    {
        ...
        // 计算旋转角度:通过三角函数避免sin、cos和arctan计算  西塔=artan(m01 / m10)
        float m_sqrt = sqrt(m01 * m01 + m10 * m10) + 1e-18;     // 避免除零
...
void BfMatch(const vector<DescType>& desc1, const vector<DescType>& desc2, vector<cv::DMatch>& matches)
{
    ...
    for(size_t i1 = 0; i1 < desc1.size(); ++i1)
    {
        ...
        cv::DMatch m{i1, 0, 256};  // 前两维为比较的描述子的下标，第三维为距离
